...
try:
    ...
    raise Exception(
        "Could not import the required modules, "
...
class PlayerBlinkGUI(tk.Frame):
    ...
    def update_configs(self, _=None):
        ...
        self.config_jsons = [
            f for f in listdir("configs") if isfile(join("configs", f))
        ]
    ...
    def create_widgets(self):
        ...
        self.new_config_button = ttk.Button(
            self, text="+", command=self.new_config, width=2
        )
        ...
        self.monitor_window = ttk.Checkbutton(
            self, text="Monitor Window", variable=self.monitor_window_var
        )
        ...
        self.monitor_blink_button = ttk.Button(
        ...
        self.menu_check = ttk.Checkbutton(
            self, text="+1 on menu close", variable=self.menu_check_var
        )
        self.menu_check.grid(column=7, row=0)
        self.menu_check_var.set(1)

        self.reident_noisy_check_var = tk.IntVar()
        self.reident_noisy_check = ttk.Checkbutton(
            self, text="Reident 1 PK NPC", variable=self.reident_noisy_check_var
        )
        ...
        self.save_button = ttk.Button(
            self, text="Save Config", command=self.save_config
        )
        self.save_button.grid(column=6, row=13, columnspan=2)

        self.raw_screenshot_button = ttk.Button(
            self, text="Raw Screenshot", command=self.save_screenshot
        )
        ...
        self.advances_increase_button = ttk.Button(
            self, text="Advance", command=self.increase_advances
        )
    ...
    def new_config(self):
        ...
        with fd.asksaveasfile(
            initialdir="./configs/", filetypes=[("JSON", ".json")]
        ) as file:
    ...
    def new_eye(self):
        ...
        self.config_json["image"] = "./" + os.path.relpath(
            fd.askopenfilename(initialdir="./images/", filetypes=[("Image", ".png")])
        ).replace("\\", "/")
    ...
    def save_config(self):
        ...
        with open(
            join("configs", self.config_combobox.get()), "w", encoding="utf-8"
        ) as file:
    ...
    def monitor_blinks(self):
        ...
        if self.previewing:
            # TODO: better way to handle this (popup ? execute "Stop Preview" job ?)
            print("Please Stop Preview first")
        if not self.monitoring and not self.previewing:
    ...
    def reidentify(self):
        ...
        if self.previewing:
            # TODO: better way to handle this (popup ? execute "Stop Preview" job ?)
            print("Please Stop Preview first")
        if not self.reidentifying and not self.previewing:
    ...
    def monitoring_work(self):
        ...
        self.rng.get_next_rand_sequence(diff * (self.config_json["npc"] + 1))
        ...
        self.s0_1_2_3.insert(
            1.0, f"{state[0]:08X}\n{state[1]:08X}\n{state[2]:08X}\n{state[3]:08X}"
        )
        self.s01_23.insert(
            1.0, f"{state[0]:08X}{state[1]:08X}\n{state[2]:08X}{state[3]:08X}"
        )
    ...
    def tidsiding_work(self):
        ...
        self.tracking = False
        munchlax_intervals = rngtool.tracking_poke_blink(
            self.player_eye,
            *self.config_json["view"],
            monitor_window=self.config_json["MonitorWindow"],
            window_prefix=self.config_json["WindowPrefix"],
    ...
    def reidentifying_work(self):
        ...
        try:
            ...
            raise Exception(
                "Cannot pull seeds from S[0-3] for reidentification, "
                "make sure they are in hex and split by line."
            ) from bad_input
        ...
        if self.reident_noisy_check_var.get():
            ...
            _, observed_intervals, offset_time = rngtool.tracking_blink(
                self.player_eye,
                *self.config_json["view"],
                monitor_window=self.config_json["MonitorWindow"],
                window_prefix=self.config_json["WindowPrefix"],
            ...
            try:
                self.rng, adv = rngtool.reidentiy_by_intervals_noisy(
                    Xorshift(*state),
                    observed_intervals,
                    search_min=int(self.reident_min.get()),
                    search_max=int(self.reident_max.get()),
                )
            except (TypeError, ValueError) as failed_deduction:
                raise Exception(
                    "Failed to reidentify from the recorded blinks."
                ) from failed_deduction
            self.timelining = True
            self.count_down = 0
        else:
            ...
            try:
                self.rng, adv = rngtool.reidentiy_by_intervals(
                    Xorshift(*state),
                    observed_intervals,
                    return_advance=True,
    ...
    def previewing_work(self):
        ...
        if self.config_json["MonitorWindow"]:
            ...
            video = WindowCapture(
                self.config_json["WindowPrefix"], self.config_json["crop"]
            )
        else:
            ...
            video.set(cv2.CAP_PROP_FRAME_WIDTH, 640)  # 1920
            video.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)  # 1080
        ...
        while self.previewing:
            ...
            if roi_w < eye_width or roi_h < eye_height:
                raise Exception(
                    "Width and Height of box cannot be smaller than selected eye image"
                )
            _, frame = video.read()
            if frame is not None:
                try:
                    roi = cv2.cvtColor(
                        frame[roi_y : roi_y + roi_h, roi_x : roi_x + roi_w],
                        cv2.COLOR_RGB2GRAY,
                    )
                except cv2.error as empty_frame:
                    raise Exception(
                        "Frame captured is empty, "
                ...
                try:
                    ...
                    raise Exception(
                        "Tried to read from out of the bounds of "
                ...
                if 0.01 < match < self.config_json["thresh"]:
                    cv2.rectangle(
                        frame, (roi_x, roi_y), (roi_x + roi_w, roi_y + roi_h), 255, 2
                    )
                else:
                ...
                if self.config_json["display_percent"] != 100:
                    ...
                    frame = cv2.resize(
                        ...
                        (
                            round(
                                frame_width * self.config_json["display_percent"] / 100
                            ),
                            round(
                                frame_height * self.config_json["display_percent"] / 100
                            ),
                        ),
                    )
    ...
    # do not update every every 100ms, only when needed
    def after_task(self):
